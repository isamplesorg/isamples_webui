...
var DEFAULT_Q = SETTINGS.defaultQuery || "*:*";
var SERVICE_ENDPOINT = SETTINGS.serviceEndpoint;
var TABLE_HEIGHT = SETTINGS.records.tableHeight || "100%";
var TABLE_ROWS = SETTINGS.records.PageSize || 100;
var COLUMNS = SETTINGS.records.columns || [
  { title: "ID", field: "id" },
...
function _doSolrLoad(url, config, params) {
  ...
  return new Promise(function(resolve2, reject) {
    try {
      resolve2(getSolrRecords(_q, _fq, _start, params.size, params.sort));
    } catch (e) {
      reject(e);
    }
  });
...
function recordsOnLoad(tabulatorDivId) {
  data_table = new TabulatorFull(`#${tabulatorDivId}`, {
    height: TABLE_HEIGHT,
    pagination: true,
...
async function showRawRecord(id) {
  ...
  await Promise.all([
    fetch(raw_url).then((response) => response.json()).then((doc) => {
      const e = document.getElementById("record_original");
      e.data = doc;
    }),
    fetch(xform_url).then((response) => response.json()).then((doc) => {
      const e = document.getElementById("record_xform");
      e.data = doc;
    }),
    fetch(solr_url).then((response) => response.json()).then((doc) => {
