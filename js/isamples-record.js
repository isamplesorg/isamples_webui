...
JSONPath.prototype._trace = function(expr, val, path, parent, parentPropName, callback, hasArrExpr, literalPriority) {
  ...
  if ((typeof loc !== "string" || literalPriority) && val && hasOwnProp.call(val, loc)) {
    ...
    this._walk(loc, x, val, path, parent, parentPropName, callback, function(m, l, _x, v, p2, par, pr, cb) {
      addRet(_this3._trace(unshift(m, _x), v, p2, par, pr, cb, true, true));
    });
  } else if (loc === "..") {
    addRet(this._trace(x, val, path, parent, parentPropName, callback, hasArrExpr));
    this._walk(loc, x, val, path, parent, parentPropName, callback, function(m, l, _x, v, p2, par, pr, cb) {
      if (_typeof(v[m]) === "object") {
        addRet(_this3._trace(unshift(l, _x), v[m], push(p2, m), v, m, cb, true));
      }
    });
  } else if (loc === "^") {
    ...
    this._walk(loc, x, val, path, parent, parentPropName, callback, function(m, l, _x, v, p2, par, pr, cb) {
      if (_this3._eval(l.replace(/^\?\(((?:[\0-\t\x0B\f\x0E-\u2027\u202A-\uD7FF\uE000-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF])*?)\)$/, "$1"), v[m], m, p2, par, pr)) {
        addRet(_this3._trace(unshift(m, _x), v, p2, par, pr, cb, true));
      }
    });
  } else if (loc[0] === "(") {
    if (this.currPreventEval) {
...
JSONPath.toPointer = function(pointer) {
  ...
  for (var i = 1; i < n; i++) {
    if (!/^(~|\^|@(?:[\0-\t\x0B\f\x0E-\u2027\u202A-\uD7FF\uE000-\uFFFF]|[\uD800-\uDBFF][\uDC00-\uDFFF]|[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?:[^\uD800-\uDBFF]|^)[\uDC00-\uDFFF])*?\(\))$/.test(x[i])) {
      p2 += "/" + x[i].toString().replace(/~/g, "~0").replace(/\//g, "~1");
    }
  }
  return p2;
};
...
var ISamplesRecord = class extends s {
  static get styles() {
    return r`
        :host {
            display: block;
  ...
  statusMessage(level = "info", message) {
    if (globalThis[this.eventBusName] !== void 0) {
      ...
      globalThis[this.eventBusName].emit("status", null, { "source": "ISamplesRecord", level, msg: message });
    }
  }
  get record() {
    return this._record;
  ...
  render() {
    if (this._record === null) {
      return p`<div>No record loaded.</div>`;
    }
    if (this._format in this._conceptMap) {
      return p`<div>
            <dl>
                ${this.conceptList.map((concept) => p`<dt title="${CONCEPTS[concept].description}">${CONCEPTS[concept].label}</dt>
